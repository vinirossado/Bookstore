...
public class BookFilterDtoValidator : AbstractValidator<BookFilterDto>
{
    ...
    {
        RuleFor(dto => dto).Must(HaveAtLeastOnePropertyWithValue).WithMessage("Needs to have at least one property with value");
    }
    
    private static bool HaveAtLeastOnePropertyWithValue(BookFilterDto dto)
    {
        return !string.IsNullOrWhiteSpace(dto.Author) ||
